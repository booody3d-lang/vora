...
export function MessageButton({
  ...
  const { execute, restrictionMessage, VisitorModal } = useGuardedAction({
    ...
    onAllowed: async () => {
      ...
      try {
        ...
        if (!res.ok) {
          // Still navigate with ?with= so the messages page can retry startConversation.
          router.push(getMessagingUrl({ targetAccountId }));
  ...
  return (
    <>
      ...
      <button
        type="button"
        disabled={disabled || busy || !targetAccountId}
        onClick={() => execute()}
        className={cn(
      ...
      {(restrictionMessage || error) && (
        <p className="mt-1 text-xs text-amber-600">{restrictionMessage || error}</p>
      )}
    </>
  );
}
