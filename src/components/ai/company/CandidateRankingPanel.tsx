  ...
  return (
    <div className="rounded-2xl border border-[#3B5998]/20 bg-gradient-to-br from-[#3B5998]/5 to-indigo-50 p-5">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-bold text-[#0F172A]">AI Candidate Ranking</h3>
        {source && (
          <span className="text-[10px] text-slate-400">{source === "openai" ? "Live AI" : "Demo"}</span>
        )}
      </div>
      <p className="mt-1 text-xs text-slate-500">
        Auto-shortlists candidates with ≥85% match quality score
      </p>
      {loading && <AILoadingSpinner />}
      {shortlisted.length > 0 && (
        <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
          <p className="text-sm font-semibold text-emerald-800">
            ⭐ {shortlisted.length} top-tier candidate{shortlisted.length > 1 ? "s" : ""} auto-flagged
          </p>
        </div>
      )}
            ...
            return (
              <div key={r.id} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-sm shadow-sm">
                <div>
                  ...
                  {r.autoShortlisted && (
                    <span className="text-[10px] font-bold text-emerald-600">AUTO-SHORTLISTED</span>
                  )}
                </div>
