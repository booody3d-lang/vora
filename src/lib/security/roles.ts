...
export const PLATFORM_OWNER_EMAIL = (
  process.env.VORA_PLATFORM_OWNER_EMAIL?.trim().toLowerCase() || "booody3d@gmail.com"
);

/** Manual secondary test account — standard registered user only */
export const MANUAL_TEST_USER_EMAIL = (
  process.env.VORA_MANUAL_TEST_USER_EMAIL?.trim().toLowerCase() || "b.3d@live.com"
);
export const MANUAL_TEST_USER_ID = "manual-test-user-b3d";

export const MAX_ADMIN_ACCOUNTS = 4;

/** Dev-only bootstrap passwords — set via environment, never hardcoded in source. */
export function getPlatformOwnerBootstrapPassword(): string | undefined {
  return process.env.VORA_PLATFORM_OWNER_BOOTSTRAP_PASSWORD?.trim() || undefined;
}

export function getManualTestUserBootstrapPassword(): string | undefined {
  return process.env.VORA_MANUAL_TEST_USER_BOOTSTRAP_PASSWORD?.trim() || undefined;
}
